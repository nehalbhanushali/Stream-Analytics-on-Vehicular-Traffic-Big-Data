...
namespace MissingCarsDataEventListener
{
    class Program
    {
        ...
        private const string EhConnectionString = "Endpoint=sb://matrix4reference.servicebus.windows.net/;SharedAccessKeyName=RootManageSharedAccessKey;SharedAccessKey=CgPsRCytXSG5ra5FA3WlawxRzTuDX+3raMXkQFjhA+M=";
        private const string EhEntityPath = "myreferenceeventhub";
        ...
        private static async Task MainAsync(string[] args)
        {
            ...
            String fullPath = @"C:\Amitha NEU\Engg Big Data\Final\SAJ\output\StolenCarData.json";
            LoadJson(fullPath);
        ...
        private static async Task SendJsonObjectsToEventHub()
        {
            ...
            {              
                ...
                try
                {
                    ...
                    var referenceJsonItem = "{ \"" + "LicensePlate" + "\" : \"" + item.LicensePlate + "\"  , " +
                       "\"" + "licenseplatePK" + "\" : \"" + item.LicensePlatePK + "\" ," +
                        "\"" + "CaseId" + "\" : \"" + item.CaseId + "\" ," +
                         "\"" + "Owner" + "\" : \"" + item.Owner + "\" ," +
                          "\"" + "RegistrationNumber" + "\" : \"" + item.RegistrationNumber + "\" ," +
                       " \"" + "licenseplateRK" + "\" : \"" + item.LicensePlateRK + "\"}";
                    Console.WriteLine(referenceJsonItem);

                    await eventHubClient.SendAsync(new EventData(Encoding.UTF8.GetBytes(referenceJsonItem)));
